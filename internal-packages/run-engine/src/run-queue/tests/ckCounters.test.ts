...
describe("CK base-queue counters", () => {
  redisTest("lengthOfQueue returns aggregate across CK variants", async ({ redisContainer }) => {
    ...
    try {
      ...
      expect(await queue.lengthOfQueue(authenticatedEnvDev, messages[0].queue, "ck-a")).toBe(2);
      expect(await queue.lengthOfQueue(authenticatedEnvDev, messages[0].queue, "ck-b")).toBe(1);
  ...
  redisTest("lazy init from pre-existing CK backlog", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
      const now = Date.now();
  ...
  redisTest("non-CK queue regression: counter never created", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
      for (let i = 0; i < 5; i++) {
        await queue.enqueueMessage({
  ...
  redisTest("mixed CK + non-CK on same base queue", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
  ...
  redisTest("length counter decrements on dequeue", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
  ...
  redisTest(
    ...
    async ({ redisContainer }) => {
      ...
      try {
        ...
        const runningCounterKey = testOptions.keys.queueRunningCounterKeyFromQueue(queueKey);
        ...
        const running = await queue.currentConcurrencyOfQueues(authenticatedEnvDev, [msg.queue]);
  ...
  redisTest("floor-at-zero protects against spurious decrements", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
      const variantA = testOptions.keys.queueKey(authenticatedEnvDev, "task/my-task", "ck-a");
  ...
  redisTest("lengthCounter has 24h TTL after lazy-init", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
      await queue.enqueueMessage({
        env: authenticatedEnvDev,
  ...
  redisTest("nack lazy-inits lengthCounter when it expired", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
      const msg = makeMessage({ runId: "r1", concurrencyKey: "ck-a" });
  ...
  redisTest(
    ...
    async ({ redisContainer }) => {
      ...
      try {
        ...
        const otherVariant = testOptions.keys.queueKey(authenticatedEnvDev, msg.queue, "ck-other");
        const ckIndexKey = testOptions.keys.ckIndexKeyFromQueue(fastVariant);
        const runningCounterKey = testOptions.keys.queueRunningCounterKeyFromQueue(fastVariant);
  ...
  redisTest("release lazy-inits runningCounter when missing", async ({ redisContainer }) => {
    const queue = createQueue(redisContainer);
    try {
      const msg = makeMessage({ runId: "r1", concurrencyKey: "ck-a" });
  ...
  redisTest("counterTtlSeconds option is honored on lazy-init", async ({ redisContainer }) => {
