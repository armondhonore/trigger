...
<Note>
  ...
  [here](/self-hosting/docker).
</Note>

<Warning>
  Security, scaling, and reliability concerns are not fully addressed here. This guide is meant for
  evaluation purposes and won't result in a production-ready deployment.
</Warning>
...
<Accordion title="Please consider these additional warnings">
  - The [docker checkpoint](https://docs.docker.com/reference/cli/docker/checkpoint/) command is an
  ...
  adding multiple worker machines, but we're working on it.
</Accordion>
