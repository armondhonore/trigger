...
<Note>
  The `prepareStep` for inner-loop compaction is automatically injected when you spread
  `chat.toStreamTextOptions()` into your `streamText` call. If you provide your own `prepareStep`
  after the spread, it overrides the auto-injected one.
</Note>
...
export const myChat = chat.agent({
  ...
  actionSchema: z.discriminatedUnion("type", [z.object({ type: z.literal("compact") })]),
...
<Note>
  The fully manual APIs only handle inner-loop compaction (between tool-call steps). For outer-loop
  coverage, use the `compaction` option on `chat.agent()`, `chat.createSession()`, or
  `MessageAccumulator`.
</Note>
