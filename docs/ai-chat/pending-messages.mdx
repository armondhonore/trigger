              ...
              pending.isInjectionPoint(part) ? (
                ...
                <div key={i}>
                  {pending.getInjectedMessages(part).map((m) => (
                    <div key={m.id} className="injected-message">
                      {m.text}
                    </div>
                  ))}
                </div>
      ...
      {pending.pending.map((msg) => (
        <div key={msg.id}>
          ...
          {msg.mode === "queued" && status === "streaming" && (
            <button onClick={() => pending.promoteToSteering(msg.id)}>Steer instead</button>
          )}
        </div>
      ))}
...
```ts
const sent = await transport.sendPendingMessage(
  chatId,
  {
