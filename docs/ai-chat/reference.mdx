...
<Note>
  **`onValidateMessages`** does not include `ctx` — it fires before message accumulation and is
  designed for pure validation/transformation of incoming messages.
</Note>
...
<Note>
  Prefer `import type {TaskRunContext} from "@trigger.dev/sdk"` in application code. Do not depend
  on `@trigger.dev/core` directly.
</Note>
...
The transport invokes `accessToken` whenever it needs a _fresh_ session-scoped PAT — initial use after no PAT is cached, or after a 401/403 from any session-PAT-authed request. The callback's job is to **return a token, not to start a run.**
...
```tsx
// Undo button
<button onClick={() => transport.sendAction(chatId, { type: "undo" })}>Undo</button>
```
