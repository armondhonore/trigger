...
<Callout type="warning">
  The deploy and list_preview_branches tools are not available when the MCP server is running with
  the `--dev-only` flag. The `--readonly` flag hides deploy, trigger_task, and cancel_run.
</Callout>
...
<Callout type="warning">
  The `start_agent_chat`, `send_agent_message`, and `close_agent_chat` tools are write operations
  and are not available in readonly mode.
</Callout>
