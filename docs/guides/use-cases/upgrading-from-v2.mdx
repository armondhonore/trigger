...
<Note>
  Don't forget to paste your own v2 code in a markdown codeblock at the bottom of the prompt before
  running it.
</Note>
...
Notice that the schema on v2 `eventTrigger` defines the payload type. In v3 that needs to be done on the TypeScript type of the `run` payload param. 2. v2 had integrations with some APIs. Any package that isn't `@trigger.dev/sdk` can be replaced with an official SDK. The syntax may need to be adapted.
For example:
v2:
...
export const parentJob = client.defineJob({
  ...
  run: async (payload, io) => {
    ...
    await exampleJob.invoke(
      { foo: "bar" },
      {
        idempotencyKey: `some_string_here_${payload.someValue}_${new Date().toDateString()}`,
      }
    );
  },
});
