...
export function emailsMatchSaved(current: string[], saved: string[]): boolean {
  return (
    JSON.stringify(normalizeEmailValues(current)) ===
    JSON.stringify(normalizeEmailValues(saved))
  );
}
...
export function buildCleanedAlertsPayloadForLimitSave(
  ...
  const thresholds = storedAlertsToThresholds(alerts, nextMode, effectiveLimitCents, planLimitCents);
...
export function normalizeBillingAlertsFromApi(apiAlerts: {
  amount: number;
  emails?: string[];
  alertLevels?: number[];
...
export function storedAlertsToThresholds(
  ...
  if (mode === "none") {
    ...
    if (amountCents === ABSOLUTE_ALERT_BASE_CENTS || alerts.amount === ABSOLUTE_ALERT_BASE_CENTS / 100) {
