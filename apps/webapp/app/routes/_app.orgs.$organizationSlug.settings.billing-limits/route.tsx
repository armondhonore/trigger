...
export const action = dashboardAction(
  ...
  if (intent === "billing-alerts") {
    ...
    if (
      submission.value.alertLevels.some((threshold) => !Number.isFinite(threshold) || threshold <= 0)
    ) {
      return redirectWithErrorMessage(
        v3BillingLimitsPath({ slug: organizationSlug }),
        request,
  ...
  if (intent === "billing-limit-resolve") {
    ...
    if (error) {
      const message =
        error instanceof Error ? error.message : "Failed to resolve billing limit";
      return redirectWithErrorMessage(
        v3BillingLimitsPath({ slug: organizationSlug }),
        request,
        message
      ...
      <PageBody scrollable ref={pageBodyRef}>
        <div className="mx-auto flex max-w-3xl flex-col gap-10 px-4 pb-4 pt-10">
          ...
          {isRecoveryMode && billingLimit.isConfigured ? (
            ...
            <BillingLimitConfigSection billingLimit={billingLimit} planLimitCents={planLimitCents} />
          )}
          <BillingAlertsSection
            alerts={alerts}
            billingLimit={billingLimit}
