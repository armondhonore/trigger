...
**Live:** [https://relaxed-weasel-trigger.cloud.nexlayer.ai](https://relaxed-weasel-trigger.cloud.nexlayer.ai)  

**Runtime:**  · **Port:** auto-detected · **Deploy branch:** nexlayer-fix

---

## How this deployment works

**trigger** is deployed on [Nexlayer](https://nexlayer.ai) — a container-native
platform where every push to `nexlayer-fix` triggers a fully automated build-and-deploy
...
application:
  ...
  - name: app
    ...
    vars:
      NODE_ENV: production
      DATABASE_URL: "postgresql://trigger:${POSTGRES_PASSWORD}@postgres.pod:5432/trigger"
      DIRECT_URL: "postgresql://trigger:${POSTGRES_PASSWORD}@postgres.pod:5432/trigger"
      ...
      MAGIC_LINK_SECRET: "a1b2c3d4e5f6a1b2c3d4"
      APP_ORIGIN: "https://relaxed-weasel-trigger.cloud.nexlayer.ai"
  - name: postgres
...
Triggers on:
- **Push** to `nexlayer-fix` → production redeploy
...
> `<podName>.pod:<port>` syntax. CI/CD runs on push to `nexlayer-fix`."*
